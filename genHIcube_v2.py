...
density_peak = 1.e+4 #nHI
density_center = 2.e+3 #nHI

r_ring_arc = 15. #arcsec
width_ring_arc = 5. #arcsec
width_center_arc = 10. #arcsec

distance = 3500. #kpc

r_ring = r_ring_arc*arcsec2radian*distance #kpc
width_ring = width_ring_arc*arcsec2radian*distance #kpc
width_center = width_center_arc*arcsec2radian*distance #kpc
...
for i in range(0,xnum):
	...
	for j in range(0,ynum):
		...
		for k in range(0,znum):
			r=math.sqrt(x**2+y**2)
			gas_map[k][j][i]  = density_peak*(math.exp( -( (r-r_ring)**2. + z**2. )/2./width_ring**2.  ))
			gas_map[k][j][i] += density_center*math.exp( -r**2. /width_center**2. )*math.exp(-z**2./2./width_ring**2.)
...
hdulist.writeto('HI_m82_torus_v1.fits')
